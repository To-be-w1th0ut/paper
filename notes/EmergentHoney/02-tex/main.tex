...
Table~\ref{tab:rq1} and Fig.~\ref{fig:adt_comparison} show overall results on the medium network ($|V|{=}200$). EmergentHoney achieves +96\% ADT, $-$71\% HIR, and +34\% TIC over the best adaptive baseline (RL-Honeypot). Compared to LLM-Static (same LLM content, no swarm), EmergentHoney achieves +34\% ADT and $-$65\% HIR, confirming that swarm self-organization is the key differentiator beyond content quality.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig4_adt_comparison.pdf}
\caption{Average Dwell Time (ADT) comparison across all methods. EmergentHoney significantly outperforms all baselines, achieving $3.24 \pm 0.16$ minutes versus the next-best LLM-Static at $2.43 \pm 0.12$ minutes. Error bars denote $\pm 1$ standard deviation over 30 independent runs.}
...
Table~\ref{tab:dei_time} and Fig.~\ref{fig:dei_evolution} present the DEI evolution over time. DEI exceeds 1.0 within the first 2 hours, confirming Theorem~\ref{thm:emergence}'s prediction. Steady-state DEI $\approx 2.0$ means the collective deception capability is \emph{double} that of the sum of individual honeypots---a strong emergence effect.
...
Table~\ref{tab:rho} shows that EmergentHoney is robust to the evaporation rate: ADT and DEI remain stable across $\rho \in [0.01, 0.50]$. Lower $\rho$ converges faster (5.7h at $\rho{=}0.01$ vs 14.2h at $\rho{=}0.50$) because slow evaporation preserves pheromone landscape structure. The recommended default $\rho = 0.05$ balances convergence speed and adaptability.
...
Table~\ref{tab:ablation} and Fig.~\ref{fig:ablation} reveal a clear component hierarchy. The pheromone mechanism is irreplaceable: removing it causes the largest ADT drop ($-$50.3\%) and the only case where $\DEI < 1$ (0.90), confirming that swarm self-organization is the foundation of emergence. LLM phenotype generation is the second most critical component ($-$37.2\% ADT), reducing HIR from 8.9\% to 22.9\% due to lower content quality. Reverse-ACO does not significantly affect ADT ($p = 0.78$) but reduces HIR from 14.0\% to 8.9\%---a 36\% relative improvement through predictive deployment that intercepts attackers before fingerprinting. Pheromone and LLM ablations show $p < 0.001$ and Cliff's $\delta = 1.0$ (Large).
...
Experiments on networks of 50--500 nodes demonstrated that EmergentHoney nearly doubles attacker dwell time (+96\%) and reduces honeypot identification rate by 71\% compared to the best adaptive baseline, with all improvements statistically significant ($p < 0.001$, Cliff's $\delta = 1.0$). The ablation study confirmed the pheromone mechanism as irreplaceable ($-$50.3\% ADT, DEI $< 1$), while the LLM phenotype generator contributes the second-largest effect ($-$37.2\% ADT). A steady-state DEI $\approx 2.0$ confirms strong positive emergence---the collective deception capability doubles relative to the sum of individual honeypots.
