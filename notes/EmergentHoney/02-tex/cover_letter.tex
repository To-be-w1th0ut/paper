...
\begin{enumerate}
    \item A pheromone-driven self-organizing honeypot topology where deployment, migration, and mutation are governed by decentralized swarm rules (Algorithm 1), with formal convergence guarantees (Theorem 1).
    \item A reverse ant colony model that interprets attacker reconnaissance trajectories as inverse foraging paths, enabling predictive deception deployment along anticipated attack vectors (Algorithm 2).
    \item The Deception Emergence Index (DEI), a formally defined metric quantifying collective emergence in deception systems, with a proof that DEI $> 1$ under mild conditions (Theorem 2).
    \item Comprehensive experimental evaluation on SDN-based testbeds (50--500 nodes) against multi-stage APT attacks, demonstrating 2.3$\times$ improvement in attacker dwell time and 68\% higher interaction rates compared to state-of-the-art baselines, with statistical significance confirmed via Wilcoxon signed-rank tests ($p < 0.001$) and large effect sizes (Cliff's $\delta > 0.71$).
\end{enumerate}
...
(i)~apply pheromone-based swarm self-organization as the \emph{operational logic} of a deception system rather than as an optimization tool;
(ii)~formally define and prove emergence in cyber deception (DEI $> 1$);
(iii)~combine reverse ACO attacker modeling with LLM-powered phenotype generation for adaptive honeypot content.

\textbf{Ethical considerations.} All experiments were conducted on isolated testbed networks with no real user data. The deception techniques described are purely defensive and comply with ethical guidelines for security research. We discuss responsible deployment considerations in Section VII.
