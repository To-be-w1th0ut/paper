...
\end{axis}
\end{tikzpicture}
\caption{Average Dwell Time comparison across all methods on the medium network. EmergentHoney (EH) achieves 36.5 min, outperforming the best baseline (RL-Honeypot) by 62.2\%.}
...
\addplot[thick, color=blue!80, mark=*, mark options={fill=blue!80}] coordinates {
    (0, 0.92) (2, 0.97) (6, 1.14) (12, 1.38) (24, 1.52) (48, 1.61) (72, 1.58)
};
...
% 标注涌现起始点
\node[font=\tiny, color=blue!80, anchor=south] at (axis cs:4.5, 1.02) {emergence onset};
\draw[->, thin, color=blue!80] (axis cs:4.5, 1.01) -- (axis cs:4.5, 0.98);

\end{axis}
\end{tikzpicture}
\caption{DEI evolution over time. DEI crosses 1.0 at $t \approx 4.5$h (confirming Theorem~1), reaches a peak of 1.61 at 48h, and stabilizes at $\approx$1.58.}
\label{fig:dei_curve}
\end{figure}
...
\addplot[thick, color=green!60!black, mark=triangle*, mark options={fill=green!50!black}, dashed] coordinates {
    (50, 2.1) (500, 7.2) (5000, 18.5)
};
\addlegendentry{$t_{\mathrm{conv}}$ (h)}

\end{axis}
\end{tikzpicture}
\caption{Scalability across network sizes. ADT and HIR remain stable (scale-invariant), while convergence time grows sub-linearly as $O(|V|^{0.6})$.}
...
% EmergentHoney ADT
\addplot[thick, color=blue!80, mark=*, mark options={fill=blue!80}] coordinates {
    (3, 28.4) (9, 19.7) (15, 25.3) (21, 24.1) (36, 22.7) (60, 21.9)
};
\addlegendentry{EmergentHoney}

% RL-Honeypot ADT
\addplot[thick, color=red!70, mark=square*, mark options={fill=red!70}] coordinates {
    (3, 22.1) (9, 14.3) (15, 12.8) (21, 11.5) (36, 9.7) (60, 8.2)
};
\addlegendentry{RL-Honeypot}

% Phase annotations
\draw[thin, dashed, gray] (axis cs:6,5) -- (axis cs:6,35);
\draw[thin, dashed, gray] (axis cs:12,5) -- (axis cs:12,35);
\draw[thin, dashed, gray] (axis cs:18,5) -- (axis cs:18,35);

\node[font=\tiny, color=gray, anchor=south] at (axis cs:3, 33) {Phase 1};
\node[font=\tiny, color=gray, anchor=south] at (axis cs:9, 33) {Phase 2};
\node[font=\tiny, color=gray, anchor=south] at (axis cs:15, 33) {Phase 3};
\node[font=\tiny, color=gray, anchor=south] at (axis cs:36, 33) {Phase 4};

% Recovery annotation
\draw[->, thin, color=blue!60] (axis cs:12, 20) -- (axis cs:15, 24.5);
\node[font=\tiny, color=blue!70, anchor=west] at (axis cs:15.5, 22) {swarm adapts};

\end{axis}
\end{tikzpicture}
\caption{Arms race dynamics against L3 adaptive attacker. EmergentHoney recovers in Phase~3 (pheromone-driven restructuring), while RL-Honeypot continuously degrades. Long-term: EmergentHoney maintains 2.67$\times$ higher ADT.}
\label{fig:arms_race}
\end{figure}
